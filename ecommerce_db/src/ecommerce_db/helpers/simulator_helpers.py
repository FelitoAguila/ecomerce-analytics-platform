...
PAYMENT_TYPES = {"credit_card": 73.9, "boleto": 19.0, "voucher": 5.6, "debit_card": 1.5}
...
def run_batch(args: argparse.Namespace, conn: psycopg.Connection) -> dict[str, int]:
    stats = {
        "new_orders": 0,
        "new_items": 0,
        "new_payments": 0,
        ...
        "status_updates": 0,
        "backdated": 0,
        "anom_missing_score": 0,
        "anom_outlier_price": 0,
        "anom_bad_status": 0,
    }

    customers = [
        r[0] for r in conn.execute("SELECT customer_id FROM customers").fetchall()
    ]
    products = [
        r[0] for r in conn.execute("SELECT product_id FROM products").fetchall()
    ]
    sellers = [r[0] for r in conn.execute("SELECT seller_id FROM sellers").fetchall()]
    ...
    for _ in range(args.orders):
        ...
        if is_backdated:
            purchase_ts = datetime(2017, 1, 1) + timedelta(days=random.randint(0, 730))
            stats["backdated"] += 1
        else:
            purchase_ts = datetime.now() - timedelta(hours=random.randint(0, 48))

        approved_ts = purchase_ts + timedelta(minutes=random.randint(5, 120))
        estimated_delivery = purchase_ts + timedelta(days=random.randint(10, 30))
        ...
        new_orders.append(
            {
                ...
                "estimated_ts": estimated_delivery,
            }
        )
    ...
    for o in new_orders:
        ...
        for item_idx in range(1, n_items + 1):
            ...
            shipping_limit = o["purchase_ts"] + timedelta(days=random.randint(3, 10))
    ...
    for o in new_orders:
        ...
        creation_date = o["delivered_ts"] + timedelta(days=random.randint(1, 30))
